...
export async function DELETE() {
  try {
    ...
    // 2. ASSET DISCOVERY: Extract keys for all images in these projects
    const allKeys = trashProjects.flatMap((project) =>
      project.slides.flatMap((slide) => {
        const assets = (slide.assets as { key?: string; url?: string }[]) || []
        return assets.map((a) => a.key || a.url).filter((k): k is string => !!k)
      })
    )

...
export interface InspirationSlide {
  ...
  // content field renamed to description
}
...
/**
 * Returns a lightweight summary of available design styles (titles and descriptions only).
 * This is used for the outline phase to save tokens while still giving the AI context.
 */
export const getInspirationsMetadata = (): string => {
  const inspirations = getInspirations()
  let output = "### 🍱 Available Design Styles:\n\n"

  inspirations.forEach((pres) => {
    output += `- **${pres.title}**: ${pres.description}\n`
    pres.slides.forEach((slide) => {
      if (slide.description) {
        output += `  - *${slide.title}*: ${slide.description}\n`
      }
    })
  })
...
export const formatInspirationsForPrompt = (): string => {
  ...
  inspirations.forEach((pres) => {
    ...
    pres.slides.forEach((slide, idx) => {
      ...
      if (slide.description)
        output += `*Design Concept/Content:* ${slide.description}\n`
